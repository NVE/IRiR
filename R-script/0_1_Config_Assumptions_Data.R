...
#### Import data ####
if (exists("use_kile")){
  dat = read.xlsx("C:/Users/clso/Downloads/Uttrekk-IR-beregning-2004-2024.xlsx") %>%
    dplyr::filter(aar %in% y.avg)
}else{
...
if (exists("use_kile")){
  kraftpris  <- read.xlsx("./Data/df_nettap.xlsx") |>
    filter(y.rc == yr) |>
    select(-y.cb, -y.rc)
}else{
...
# Setter KILE til 0 hvis use KILE-eksistere
if (exists("use_kile") & !use_kile){
  dat$ld_cens <- 0
  dat$rd_cens <- 0
  dat$t_cens <- 0
...
if (exists("panel")){ # CLSO: Har tilføyet dette midlertidig, men bør justeres for å sikre at all data inngår når vi beregner inntektsrammer tilbake i tid. 
  dat <- dat %>%
