...
if(BS.new == 1) {
  ...
  ld_bs = read.csv("./Harmony/Bootstrap/ld_bs_pre.csv",sep=",")
}

if(BS.new == 1 ) {
  names(ld_bs) = "ld_bs.correst"
  ld_bs = add_rownames(ld_bs, "id")
  write.csv(ld_bs, "./Harmony/Bootstrap/ld_bs_pre.csv", row.names = FALSE)
}
...
if(BS.new == 1) {
  ...
  rd_bs = read.csv("./Harmony/Bootstrap/rd_bs_pre.csv",sep=",")
}
...
if(BS.new == 1 ) {
  ...
  write.csv(rd_bs, "./Harmony/Bootstrap/rd_bs_pre.csv", row.names = FALSE)
}
...
ld_EVAL$ld_eff.s2.avg = Zvar2(x = X.avg.ld, eff = eff.avg.avg.ld, id = names(X.avg.ld),
                             lambda = ld_lambda.avg, coeff = ldz.coeff, z = Geovar.ldz)$eff.corr
...
rd_EVAL$rd_eff.s2.avg = Zvar2(x = X.avg.rd, eff=eff.avg.avg.rd, id=names(X.avg.rd),
                             lambda = rd_lambda.avg, coeff=rdz.coeff, z=Geovar.r)$eff.corr
