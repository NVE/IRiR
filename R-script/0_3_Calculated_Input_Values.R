...
dat$rd_cga.tidl.coord = dat$rd_cga + dat$rd_cga_tidl + dat$rd_coord
...
v_cpi = c("ld_cens", "rd_cens", "t_cens")
        ...
        for(c in 1:length(v_cpi))
          for(r in 1:nrow(dat))
            dat[r, fp_v_cpi[c]] = dat[r, v_cpi[c]] * cpi[as.character(year(y.cb))] / cpi[as.character(dat[r, "y"])]
        
        # CPI.L adjustment
        v_cpi.l = c("ld_391", "rd_391", "t_391", "ld_pens", "rd_pens", "t_pens", "ld_pens.eq", "rd_pens.eq", "t_pens.eq", 
                    "ld_impl", "rd_impl", "t_impl", "ld_OPEXxS", "rd_OPEXxS", "rd_cga.tidl.coord", "t_OPEXxS", "ld_sal", 
        ...
        for(c in 1:length(v_cpi.l))
          for(r in 1:nrow(dat))
            dat[r, fp_v_cpi.l[c]] = dat[r, v_cpi.l[c]] * cpi.l[as.character(year(y.cb))] / cpi.l[as.character(dat[r, "y"])]
