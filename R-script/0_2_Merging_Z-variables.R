...
ap.t_2 <- ap.t_2 %>% 
  mutate(ap.t_2 = if_else(id == 269, 0.537166713, ap.t_2))
...
fusjon_Linja = c(912631532, 984882114) # Linja AS = Linja AS + Mørenett AS
fusjon_NordvestNett = c(921699905, 980824586) # Nordvest Nett AS = Nordvest Nett AS + Sandøy Nett AS

list <- list() 

list$Linja = merge_ldz_rdz(comps = fusjon_Linja, new.org = 982974011, new.id = 269, new.name = "fusjonert_Linja", sum_variables = harm.var_sum, ldz_weighted.var = ldz_harm.var_gc, df = dat_geo)$mds
list$NordvestNett = merge_ldz_rdz(comps = fusjon_NordvestNett, new.org = 980824586, new.id = 613, new.name = "fusjonert_NordvestNett", sum_variables = harm.var_sum, ldz_weighted.var = ldz_harm.var_gc, df = dat_geo)$mds
