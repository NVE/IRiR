...
y.cb <-  as.yearqtr("2023-01-01") # Settes til 1. januar for ?ret for rapportering
y.rc <- year(y.cb+2) # Inntektsrammeår = cost base year + 2 år
...
if (decision == 0){
  future <- read_xlsx("./market_prices_2024-09-30.xlsx") #Oppdatert 30.09.24 PEUN
}

# Rente for forrige inntektsrammeaar (t-1)
swap_y.rc_1 <- 3.78          # Oppdatert 20.11.24 PEUN 
kreditpremie_y.rc_1 <- 0.97  # Oppdatert 20.11.24 PEUN
 
# Rente for inntektsrammeaaret (t)
swap_y.rc <- 3.51            # Oppdatert 20.11.24 PEUN
kreditpremie_y.rc <- 0.97    # Oppdatert 20.11.24 PEUN

#### 12824: Elektrisitetsbalanse (MWh) #### 
# Download data 
...
# Beregner forbruksvekter per mnd for y.cb
forbruk_per_mnd <- as.data.frame(px_data, 
  ...
  group_by(year(date)) %>% 
  mutate(vekt = `Elektrisk kraft`/sum(`Elektrisk kraft`)) %>% 
  ungroup() %>% 
  arrange(date)
...
# # Hvis forhandsbestemt dato velges valutakurs fra denne, ellers velges siste tilgjengelige valutakurs
# if (exists("time_period_currency")){
#   url <- paste0("https://data.norges-bank.no/api/data/EXR/B.EUR.NOK.SP?format=sdmx-generic-2.1&startPeriod=",time_period_currency,"&endPeriod=",time_period_currency,"&locale=no")
#   valutakurs <- readSDMX(url) %>% 
#     as.data.frame(api) %>% 
#     dplyr::rename(OBS_VALUE = obsValue,
...
if (decision == 0){
  ...
  future <- as.data.frame(future) %>% 
    ...
    filter(as.numeric(y) == as.numeric(y.cb)+2) # Filtrer ut irrelevante aar
}

#### Beregne forbruksvekter per kvartal ####
...
#### Referansepris kraft ####
if (decision == 0){ # Beregne referansepris for varsel
  ...
  } else { # Beregne referansepris på kraft brukt i vedtak
  referansepris_kraft <- kraftpriser_mnd %>% 
    ...
    nettap <- read_xlsx("Data/BaseData/nettap_prisomraader.xlsx", # denne bør byttes med en spørring mot DWH
...
#     } else {            
#       aar >= 2004 & aar <= y.rc # Hvis vedtak, sletter observasjoner før 2004 og etter y.rc
#     }
#   ) 
...
# Legger til kpi for inntektsrammeåret fra SSB tabell 03014 for vedtak-modus (overskriver verdi fra tabell 12880)
if(decision==1){
...
# Pga ulike opplastingstidspunkt til DWH blir det to rader per konsesjonær. 
# Dette pga trøbbel med opplasting av 'ldz_mgc', fyller NA-radene med riktig verdi for konsesjonæren,
