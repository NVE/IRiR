...
if (exists("panel")) {
  sysp.t_2 = read.xlsx("./Data/systempris_sysp.t_2.xlsx") %>%
    filter(y.cb == year(.env$y.cb)) %>%
    pull(sysp.t_2)
  sysp.t_2
}
...
pnl.rc <- read_xlsx("./Data/df_nettap.xlsx") |>
  filter(y.rc == yr)
...
for (yr in years_back) {
  ...
  if (next_year %in% kpi_l$aar) {
    
    next_value <- kpi_l$index[kpi_l$aar == next_year]
    g <- growth_old$g_old[growth_old$aar == next_year]
...
# faktor
y.cb.cpi.factor =
  cpi[as.character(y.rc)] /
  cpi[as.character(year(y.cb))]
...
dat <- dat %>% 
  ...
  filter(name_ir >= 2004) %>% # endret fra 2009 til 2004
  filter(Rad == "TRUE") %>%
...
if (!exists("use_kile")){
  ...
  dat_geo <- dat[dat$y.cb == max(years), ]
}
