...
# Comp 1 
merg.comp1 = c(979422679) # Skagerak Nett AS
# Comp 2
merg.comp2 = c(982699614) # Løvenskiol Fossum Kraft (LFK)


merg.comps = c(merg.comp1, merg.comp2)

comp.name = c("Skagerak Nett AS - fusjonert med LFK")

nytt_norgesnett = c(974521539, 980234088, 982560713)
fusjon_Haugaland_SKL = c(970983074, 992860634)
fusjon_Nordlandsnett = c(990892679, 915558119)
fusjon_Glitre = c(880340492, 815299302, 981915550)
...
source("./R-script/Harmony/H_0_1_Config_Assumptions_Data_Merge2016.R")
...
Haugaland_data = merge_NVE(comps = fusjon_Haugaland_SKL, new.org = 995995995, new.id = 503, new.name = "Haugaland AS (Fusjonert)",
...
dat = bind_rows(dat, Haugaland_data)
dat = dat[!(dat$orgn %in% fusjon_Haugaland_SKL),]



#Hensyntar fusjon av Norgesnett
NyttNorgesnett_data = merge_NVE(comps = nytt_norgesnett, new.org = 997997997, new.id = 14, new.name = "Norgesnett AS (Fusjonert)",
                         sum_variables = harm.var_sum, ldz_weighted.var = ldz_harm.var_gc,
                         rdz_weighted.var = rdz_harm.var_gc,merge.mean = merge.pr, df =dat)$mds

dat = bind_rows(dat, NyttNorgesnett_data)
dat = dat[!(dat$orgn %in% nytt_norgesnett),]


#Hensyntar fusjon av Nordlansnett og Rødøy Lurøy Kraftlag
NyttNordlandsnett_data = merge_NVE(comps = fusjon_Nordlandsnett, new.org = 996996996, new.id = 726, new.name = "Nordlandsnett (Fusjonert)",
                                sum_variables = harm.var_sum, ldz_weighted.var = ldz_harm.var_gc,
                                rdz_weighted.var = rdz_harm.var_gc,merge.mean = merge.pr, df =dat)$mds

dat = bind_rows(dat, NyttNordlandsnett_data)
dat = dat[!(dat$orgn %in% fusjon_Nordlandsnett),]

#Hensyntar fusjon Glitre 
Glitre_data = merge_NVE(comps = fusjon_Glitre, new.org = 994994994, new.id = 615, new.name = "Glitre AS (Fusjonert)",
                        sum_variables = harm.var_sum, ldz_weighted.var = ldz_harm.var_gc,
                        rdz_weighted.var = rdz_harm.var_gc,merge.mean = merge.pr, df =dat)$mds

dat = bind_rows(dat, Glitre_data)
dat = dat[!(dat$orgn %in% fusjon_Glitre),]
...
rm(list = ls()[!ls() %in% c('comp_pre', 'RC_pre', 'BS.new', 'BS.ite', 'merg.comps',
                            ...
                            'nytt_norgesnett', 'fusjon_Haugaland_SKL', 'fusjon_Nordlandsnett', 'fusjon_Glitre',
                            'KeyFigOrg_Pre','start.time')])
...
new_data = merge_NVE(comps = merg.comps, new.org = 999999999, new.id = 999, new.name = "Skagerak Nett AS - fusjonert med LFK",
                       sum_variables = harm.var_sum, ldz_weighted.var = ldz_harm.var_gc,
                       rdz_weighted.var = rdz_harm.var_gc,merge.mean = merge.pr, df =dat)$mds
